...
int main(int argc, char* argv[]){
    ...
    if (argc > 4){
        cout << "usage: etbr_thread_cmd circuit_name [-nq reduced_order]\n";
		exit(-1);
		pthread_exit((void*) 0);
	}else if (argc == 4){
	...
	cs_dl* Gs, *Cs, *Bs;
	//stamp(cktname, nL, nVS, nNodes, tstep, tstop, VS, IS, G,  C,  B, nodePool);
	...
	if (npart == 1){
	  ...
	  if (dc_sign == 1){		
		...
		if (thread_version){
		  etbr_thread(Gs, Cs, Bs, VS, nVS, IS, nIS, tstep, tstop, q, 
	  ...
	  out_GC_file.close();
	  UF_long *node_part = new UF_long[Gs->m];
	  UF_long *part_size = new UF_long[npart+1];
	  UF_long *mat_pinv = new UF_long[Gs->m];
	  UF_long *mat_q = new UF_long[Gs->m];
	  partition_run_time.start();
	  partition_cpu_time.start();
	  partition3(As, npart, nNodes, part_size, node_part, mat_pinv, mat_q);
	  cs_dl_spfree(As);
