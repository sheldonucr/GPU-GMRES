		...
		Preconditioner &preconditioner)// n: rowNum, m: restart threshold
{
	...
	if ((resid = norm2(r, n) / normb) <= *tol) {
		...
		*max_iter = 0;
		...
		free(H);
		free(v);
#ifdef GMRES_SHOW_RESID_PROGRESS
		cout<<endl;
#endif
		return 0;
	}
...
// An adaption from iterative_solvers_implementation.h : 815
int 
GMRESilu(const float *val, const  int *rowIndices, const  int *indices,
         float *x, const float *b, const  int n,
         const  int m, int *max_iter, float *tol, 
         Preconditioner &preconditioner)// n: rowNum, m: restart threshold
{
  //printf("         using GMRESilu\n");
  float resid;
  int i, j = 1, k;
  ...
  // XXLiu:  normb = norm( M.solve(b) )
  //float normb = norm2(b, n);
  //KuangYa: preconditioner.HostPrecond(b, bb);
  preconditioner.HostPrecond_rhs(b, bb);
  float normb = norm2(bb, n);
  if(normb == 0.0)  normb = 1.0;
  
  // XXLiu: Vector r = M.solve(b - A * x);
  // -- use initial guess
  preconditioner.HostPrecond_starting_value(x, y);
  sgemv(rr, val, rowIndices, indices, -1.0, x, 1, b, n, n);
  // -- use no initial guess
  // sgemv(rr, val, rowIndices, indices, -1.0, x, 1, b, n, n);  //KuangYa: preconditioner.HostPrecond(rr, r);
  // --
  preconditioner.HostPrecond_rhs(rr, r);
  ...
  while (j <= *max_iter) {
    ...
    for (i = 0; i < m && j <= *max_iter; i++, j++) {
      // XXLiu: w = M.solve(A * v[i]);
      //computeSpMV(w, val, rowIndices, indices, v+i*n, n);
      preconditioner.HostPrecond_right(v+i*n, w);
      computeSpMV(ww, val, rowIndices, indices, w, n);
      preconditioner.HostPrecond_left(ww, w);
      //KuangYa: computeSpMV(ww, val, rowIndices, indices, v+i*n, n);
      //KuangYa: preconditioner.HostPrecond(ww, w);
...
#endif
        //printf("HOST---BREAK: %6.4e\n",resid);
        Update(y, i, H, m, s, v, n);
                                
        *tol = resid;
        *max_iter = j;
                                
        preconditioner.HostPrecond_right(y, x);
...
int 
GMRESilu_GPU(float *d_val, int *d_rowIndices, int *d_indices, int nnz,
             float *d_x, float *d_b, const  int n,
             const  int m, int *max_iter, float *tol, 
             Preconditioner &preconditioner)
{
  ...
  if( status != CUSPARSE_STATUS_SUCCESS ) {
    printf( "CUSPARSE Library create handle failed.\n");
    return 1;
  }
  status = cusparseCreateMatDescr( &descr );
  if( status != CUSPARSE_STATUS_SUCCESS ) {
    printf( "CUSPARSE create mat descr failed.\n");
    return 1;
  }
  ...
  checkCudaErrors(cudaMalloc((void**) &d_rr, n*sizeof(float)));
  checkCudaErrors(cudaMalloc((void**) &d_bb, n*sizeof(float)));
  checkCudaErrors(cudaMalloc((void**) &d_y, n*sizeof(float)));
  */
  float *d_r=preconditioner.d_r, *d_rr=preconditioner.d_rr,
    *d_bb=preconditioner.d_bb, *d_y=preconditioner.d_y; // use preallocated memory instead

  /* // XXLiu: On 2012-10-19, found this one is not used, and it causes leaks.
     float *d_temp;
     checkCudaErrors(cudaMalloc((void**)& d_temp, n*sizeof(float)));
  */
  
  // XXLiu:  normb = norm( M.solve(b) )
  preconditioner.DevPrecond_rhs(d_b, d_bb);
  float normb = cublasSnrm2(n, d_bb, 1);
  if(normb == 0.0)  normb = 1.0;

  float neg_one=-1.0, one=1.0, zero=0.0;
  // XXLiu: Vector r = M.solve(b - A * x);
  preconditioner.DevPrecond_starting_value(d_x, d_y);

  cudaMemcpy(d_rr, d_b, n*sizeof(float), cudaMemcpyDeviceToDevice);
  status = cusparseScsrmv( handle, CUSPARSE_OPERATION_NON_TRANSPOSE, n, n, nnz,
                           &neg_one, descr, d_val, d_rowIndices, d_indices,
                           d_x , &one , d_rr);
  if( status != CUSPARSE_STATUS_SUCCESS ) {
    printf("cuSparse csrmv in GMRESilu_GPU failed.\n");
    return 1;
  }
  preconditioner.DevPrecond_rhs(d_rr, d_r);
  float beta = cublasSnrm2(n, d_r, 1);
  if ((resid = beta / normb) <= *tol) {
    *tol = resid;
    *max_iter = 0;
  ...
  float *d_v, *d_w, *d_ww;
  checkCudaErrors(cudaMalloc((void**) &d_v, (m+1)*n*sizeof(float)));
  checkCudaErrors(cudaMalloc((void**) &d_w, n*sizeof(float)));
  checkCudaErrors(cudaMalloc((void**) &d_ww, n*sizeof(float)));
  */
  float *s=preconditioner.s, *cs=preconditioner.cs,
    *sn=preconditioner.sn, *H=preconditioner.H; // use preallocated memory instead
  float *d_v=preconditioner.d_v, *d_w=preconditioner.d_w, *d_ww=preconditioner.d_ww;
  ...
  while (j <= *max_iter) {
    ...
    for (i = 0; i < m && j <= *max_iter; i++, j++) {
      // XXLiu: w = M.solve(A * v[i]);
      preconditioner.DevPrecond_right(d_v+i*n, d_w);
      status = cusparseScsrmv( handle, CUSPARSE_OPERATION_NON_TRANSPOSE, n, n, nnz,
                               &one, descr, d_val, d_rowIndices, d_indices,
                               d_w, &zero, d_ww);
      preconditioner.DevPrecond_left(d_ww, d_w);
...
#endif
        //printf("DEV---BREAK: %6.4e\n",resid);
        Update_GPU(d_y, i, H, m, s, d_v, n);                     
        preconditioner.DevPrecond_right(d_y, d_x);
    ...
    Update_GPU(d_y, m-1, H, m, s, d_v, n);
    preconditioner.DevPrecond_right(d_y, d_x);
