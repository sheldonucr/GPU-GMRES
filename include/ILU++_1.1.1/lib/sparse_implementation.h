...
template<class T> T vector_dense<T>::operator * (vector_dense const &v) const {   // Scalar-Product
     ...
     #endif
     T h = (T) 0;
     for (Integer i=0;i<size;i++) h += data[i]*conj(v.data[i]);
     return h;
  }
...
template<class T> void vector_dense<T>::read_hb(char* filename, Integer k){
   try {
     std::cout<<"vector_dense<T>::read_hb: vector does not appear to be of double type, but vector read is double. This may cause unpredictable results. Keep your fingers crossed"<<std::endl;
     int M, N, nonzeros, Nrhs;
     char *Type;
     int returned_vectors;
     T* rhsdata;
     readHB_info(filename, &M, &N, &nonzeros, &Type, &Nrhs);
     if(k<1 || k> Nrhs){
         std::cerr<<"vector_dense<T>::read_hb: No vector has specified index. Returning vector of dimension 0"<<std::endl;
         resize_without_initialization(0);
         return;
     }
     if(Nrhs == 1){
         resize_without_initialization(N);
         returned_vectors = readHB_aux_double(filename,'F',data);
         if(returned_vectors != 1) {
             std::cerr<<"vector_dense<T>::read_hb: reading vector not successful. Returning vector of dimension 0"<<std::endl;
             resize_without_initialization(0);
             return;
         }
     } else {
         if(N <= 0 || Nrhs <= 0) rhsdata = 0; 
         else rhsdata = new T[N*Nrhs];
         resize_without_initialization(N);
         readHB_aux_double(filename,'F',rhsdata);
         for(Integer i = 0; i<N; i++) data[i] = rhsdata[k*N+i];
         if(rhsdata != 0) delete [] rhsdata;
     }
   }
   catch(std::bad_alloc){
       std::cerr<<"vector_dense<T>::read_hb: Error allocating memory. Returning vector of dimension 0."<<std::endl;
       resize_without_initialization(0);
       throw iluplusplus_error(INSUFFICIENT_MEMORY);
   }
   catch(iluplusplus_error ippe){
       std::cerr<<"vector_dense<T>::read_hb "<<ippe.error_message()<<"Returning vector of dimension 0"<<std::endl;
       resize_without_initialization(0);
       throw;
   }
}

template<class T> void vector_dense<T>::read_hb(std::string filename, Integer k){
   T* rhsdata = 0;
   try {
     std::cout<<"vector_dense<T>::read_hb: vector does not appear to be of double type, but vector read is double. This may cause unpredictable results. Keep your fingers crossed"<<std::endl;
     int M, N, nonzeros, Nrhs;
     ...
     if(Nrhs == 1){
         resize_without_initialization(N);
         returned_vectors = readHB_aux_double(filename.c_str(),'F',data);
         if(returned_vectors != 1) {
             std::cerr<<"vector_dense:<T>:read_hb: reading vector not successful. Returning vector of dimension 0"<<std::endl;
             resize_without_initialization(0);
         }
         ...
         resize_without_initialization(N);
         returned_vectors = readHB_aux_double(filename.c_str(),'F',rhsdata);
         if(returned_vectors < k) {
             std::cerr<<"vector_dense<T>::read_hb: reading vector not successful. Returning vector of dimension 0"<<std::endl;
             resize_without_initialization(0);
             if(rhsdata != 0) delete [] rhsdata;
             return;
...
template<> void vector_dense<double>::read_hb(char* filename, Integer k){
   double* rhsdata=0;
   try {
     int M, N, nonzeros, Nrhs;
     char *Type;
     ...
     readHB_info(filename, &M, &N, &nonzeros, &Type, &Nrhs);
     if(k<1 || k> Nrhs){
         std::cerr<<"vector_dense<double>::read_hb: No vector has specified index. Returning vector of dimension 0"<<std::endl;
         resize_without_initialization(0);
         return;
     }
     if(Nrhs == 1){
         ...
         returned_vectors = readHB_aux_double(filename,'F',data);
         if(returned_vectors != 1) {
             std::cerr<<"vector_dense<double>::read_hb: reading vector not successful. Returning vector of dimension 0"<<std::endl;
             resize_without_initialization(0);
             return;
         }
     } else {
         if(N <= 0 || Nrhs <= 0) rhsdata = 0; 
         else rhsdata = new double[N*Nrhs];
         resize_without_initialization(N);
         returned_vectors = readHB_aux_double(filename,'F',rhsdata);
         if(returned_vectors <k){
             std::cerr<<"vector_dense<double>::read_hb: reading vector not successful. Returning vector of dimension 0"<<std::endl;
             resize_without_initialization(0);
             if(rhsdata != 0) delete [] rhsdata;
       ...
       std::cerr<<"vector_dense<double>::read_hb: Error allocating memory. Returning vector of dimension 0."<<std::endl;
       resize_without_initialization(0);
       if(rhsdata != 0) delete [] rhsdata;
       throw iluplusplus_error(INSUFFICIENT_MEMORY);
   }
   catch(iluplusplus_error ippe){
       std::cerr<<"vector_dense<double>::read_hb "<<ippe.error_message()<<"Returning vector of dimension 0"<<std::endl;
       resize_without_initialization(0);
       if(rhsdata != 0) delete [] rhsdata;
       resize_without_initialization(0);
       throw;
...
template<> void vector_dense<double>::read_hb(std::string filename, Integer k){
   double* rhsdata=0;
   try {
     int M, N, nonzeros, Nrhs;
     char *Type;
...
template<> void vector_dense<std::complex<double> >::read_hb(char* filename, Integer k){
   double* rhsdata=0;
   try {
     int M, N, nonzeros, Nrhs;
     char *Type;
     ...
     readHB_info(filename, &M, &N, &nonzeros, &Type, &Nrhs);
     if(k<1 || k> Nrhs){
         std::cerr<<"vector_dense<std::complex<double> >::read_hb: No vector has specified index. Returning vector of dimension 0"<<std::endl;
         resize_without_initialization(0);
         return;
     }
     if(N <= 0 || Nrhs <= 0) rhsdata = 0; 
     else rhsdata = new double[2*N*Nrhs];
     resize_without_initialization(N);
     returned_vectors = readHB_aux_double(filename,'F',rhsdata);
     if(returned_vectors <k){
         std::cerr<<"vector_dense<std::complex<double> >::read_hb: reading vector not successful. Returning vector of dimension 0"<<std::endl;
         resize_without_initialization(0);
         if(rhsdata != 0) delete [] rhsdata;
         return;
     }
     for(Integer i = 0; i<N; i++) data[i] = std::complex<double>(rhsdata[2*(k*N+i)],rhsdata[2*(k*N+i)+1]);
     if(rhsdata != 0) delete [] rhsdata;
   }
   catch(std::bad_alloc){
       std::cerr<<"vector_dense<std::complex<double> >::read_hb: Error allocating memory. Returning vector of dimension 0."<<std::endl;
       resize_without_initialization(0);
       if(rhsdata != 0) delete [] rhsdata;
       throw iluplusplus_error(INSUFFICIENT_MEMORY);
   }
   catch(iluplusplus_error ippe){
       std::cerr<<"vector_dense<std::complex<double> >::read_hb "<<ippe.error_message()<<"Returning vector of dimension 0"<<std::endl;
       resize_without_initialization(0);
       if(rhsdata != 0) delete [] rhsdata;
       resize_without_initialization(0);
       throw;
...
template<> void vector_dense<std::complex<double> >::read_hb(std::string filename, Integer k){
   double* rhsdata=0;
   try {
     int M, N, nonzeros, Nrhs;
     char *Type;
...
template<class T> Real vector_dense<T>::norm2_squared() const {          // returns the 2-norm of a vector
     Real h = (T) 0;
     for (Integer i=0;i<size;i++) h += fabs(conj(data[i])*data[i]);
     return h;
  }
...
void quicksort(index_list& v, index_list& list, const index_list& permutation, Integer left, Integer right){
     ...
     Integer m;
     if(left<right){
         m=permutation.get(v[left]);
         i=left;
         j=right;
         while(i<=j){
             while(permutation.get(v[i])<m) i++;
             while(permutation.get(v[j])>m) j--;
             if(i<=j){
                 v.switch_index(i,j);
                 list.switch_index(i,j);
                 i++;
...
template<class T> void matrix_sparse<T>::read_hb(char* filename){
     std::cout<<"matrix_sparse::read_hb: Coefficient matrix does not appear to be of double or complex<double> type, but reading HB files requires double. This may cause unpredictable results. Keep your fingers crossed"<<std::endl;
     int M, N, nonzeroes;
     // delete and set to zero, so that if reading fails, reformat knows that fields have already been deleted.
     if(pointer != 0){ delete [] pointer; pointer = 0;}
     if(indices != 0){ delete [] indices; indices = 0;}
     if(data != 0) { delete [] data; data = 0;}
     int successful_read;
     successful_read = readHB_newmat_double(filename, &M, &N, &nonzeroes, &pointer, &indices, &data);
     if(successful_read == 1){
         number_rows = M;
         number_columns = N;
         nnz = nonzeroes;
         pointer_size = number_columns+1;
         orientation = COLUMN;
     } else {
         std::cerr<<"matrix_sparse::read_hb: Reading "<<filename<<" not successful. Returning empty matrix."<<std::endl;
         // these are the current sizes because of the deletes above
         number_rows = 0;
         number_columns = 0;
         nnz = 0;
         pointer_size = 0;
         orientation = COLUMN;
         // still need to resize and initialize pointer
         reformat(0,0,0,COLUMN);
     }
  }

template<class T> void matrix_sparse<T>::read_hb(std::string filename){
     std::cout<<"matrix_sparse::read_hb: Coefficient matrix does not appear to be of double or complex<double> type, but reading HB files requires double. This may cause unpredictable results. Keep your fingers crossed"<<std::endl;
     int M, N, nonzeroes;
     // delete and set to zero, so that if reading fails, reformat knows that fields have already been deleted.
     if(pointer != 0){ delete [] pointer; pointer = 0;}
     ...
     if(data != 0) { delete [] data; data = 0;}
     int successful_read;
     successful_read = readHB_newmat_double(filename.c_str(), &M, &N, &nonzeroes, &pointer, &indices, &data);
     if(successful_read == 1){
         number_rows = M;
         number_columns = N;
         nnz = nonzeroes;
...
template<> void matrix_sparse<double>::read_hb(char* filename){
     int M, N, nonzeroes;
     // delete and set to zero, so that if reading fails, reformat knows that fields have already been deleted.
     if(pointer != 0){ delete [] pointer; pointer = 0;}
     if(indices != 0){ delete [] indices; indices = 0;}
...
template<> void matrix_sparse<double>::read_hb(std::string filename){
     int M, N, nonzeroes;
     // delete and set to zero, so that if reading fails, reformat knows that fields have already been deleted.
     if(pointer != 0){ delete [] pointer; pointer = 0;}
     if(indices != 0){ delete [] indices; indices = 0;}
...
template<> void matrix_sparse<std::complex <double> >::read_hb(char* filename){
     #ifdef DEBUG
         std::cout<<"matrix_sparse<std::complex <double> >::read_hb:"<<std::endl;
         std::cout<<"       WARNING:"<<std::endl;
         std::cout<<"          converting a double* to std::complex<double>* using a reinterpret_cast"<<std::endl;
         std::cout<<"          this usually works, but is not standard...."<<std::endl;
     #endif
     int M, N, nonzeroes;
     // delete and set to zero, so that if reading fails, reformat knows that fields have already been deleted.
     if(pointer != 0){ delete [] pointer; pointer = 0;}
     ...
     double* realdata = 0;
     int successful_read;
     successful_read = readHB_newmat_double(filename, &M, &N, &nonzeroes, &pointer, &indices, &realdata);
     if(successful_read == 1){
         number_rows = M;
         number_columns = N;
         nnz = nonzeroes;
         pointer_size = number_columns+1;
         orientation = COLUMN;
         data = reinterpret_cast< std::complex<double>* > (realdata);
     } else {
         std::cerr<<"matrix_sparse::read_hb: Reading "<<filename<<" not successful. Returning empty matrix."<<std::endl;
         // these are the current sizes because of the deletes above
         number_rows = 0;
...
template<> void matrix_sparse<std::complex <double> >::read_hb(std::string filename){
     #ifdef DEBUG
         std::cout<<"matrix_sparse<std::complex <double> >::read_hb:"<<std::endl;
         std::cout<<"       WARNING:"<<std::endl;
         std::cout<<"          converting a double* to std::complex<double>* using a reinterpret_cast"<<std::endl;
     ...
     double* realdata = 0;
     int successful_read;
     successful_read = readHB_newmat_double(filename.c_str(), &M, &N, &nonzeroes, &pointer, &indices, &realdata);
     if(successful_read == 1){
         number_rows = M;
         number_columns = N;
         nnz = nonzeroes;
...
template<class T> void matrix_sparse<T>::write_hb(std::string filename, std::string title, std::string key) const {
     double* rhs = 0;
     double* guess = 0;
     double* exact = 0;
     char type[] = "R";
     ...
     if(orientation==ROW){
         writeHB_mat_double(filename.c_str(),number_rows,number_columns,nnz,pointer,indices,data,0,rhs,guess,exact,title.c_str(),key.c_str(),type,ptrfmt,indfmt,valfmt,rhsfmt,"FGN");
     } else {
         Acol.change_orientation_of_data(*this);
         writeHB_mat_double(filename.c_str(),Acol.number_rows,Acol.number_columns,Acol.nnz,Acol.pointer,Acol.indices,Acol.data,0,rhs,guess,exact,title.c_str(),key.c_str(),type,ptrfmt,indfmt,valfmt,rhsfmt,"FGN");
     }
 }
...
template<class T> void matrix_sparse<T>::triangular_solve(special_matrix_type form, matrix_usage_type use, const index_list perm, const vector_dense<T>& b, vector_dense<T>& x) const{
  try {
    ...
    {   // untested, requires inverse perm (called perm as well)
        ...
        for(k=0;k<number_columns;k++){
            y[k] /= data[pointer[k]];
            for(j=pointer[k]+1;j<pointer[k+1];j++) y[indices[j]] -= data[j]*y[perm.get(k)];
         }
        for(k=0;k<number_columns;k++) x[k]=y[perm.get(k)];
        #ifdef VERYVERBOSE
             time_2 = clock();
             time = ((Real)time_2-(Real)time_1)/(Real)CLOCKS_PER_SEC;
             std::cout<<std::endl<<"          triangular_solve time: "<<time<<std::endl<<std::flush;
         ...
         ((form==PERMUTED_LOWER_TRIANGULAR) && (orientation==COLUMN) && (use==TRANSPOSE)) )
    {
        ...
        for(k=number_rows-1;k>=0;k--){
            ...
            x[perm.get(k)] = y[k]/data[pointer[k]];
        }   // end for k
        #ifdef VERYVERBOSE
             time_2 = clock();
             time = ((Real)time_2-(Real)time_1)/(Real)CLOCKS_PER_SEC;
...
template<class T> bool matrix_sparse<T>::ILUT2(const matrix_sparse<T>& A, matrix_sparse<T>& U, Integer max_fill_in, Real threshold, Real& time_self){
      clock_t time_begin, time_end;
      time_begin=clock();
      if(threshold > 500) threshold = 0.0;
      else threshold=std::exp(-threshold*std::log(10.0));
...
std::cout<<"ListL"<<std::endl<<list_L<<std::endl;
          // (11.) Copy values to L:
          for(j=0;j<list_L.dimension();j++){
              data[pointer[i]+j] = w.read(list_L[j]);
              indices[pointer[i]+j] = list_L[j];
std::cout<<"copied "<<w.read(list_L[j])<<" to position "<<indices[pointer[i]+j]<<std::endl;
          } // end for j
          data[pointer[i]+list_L.dimension()]=1.0;
          indices[pointer[i]+list_L.dimension()]=i;
          pointer[i+1]=pointer[i]+list_L.dimension()+1;
    ...
    try {
      ...
      for(k=0;k<n;k++){
        ...
        unused_rows.set(selected_row)=false;
          ...
          #endif
          // (6.) sort and copy data to U; update information for accessing columns of U
          z.take_single_weight_largest_elements_by_abs_value_with_threshold_pivot_last(list_U,weights_U,max_fill_in,threshold,perm.get(k),perm_tol);
              // dropping too stringent?
          if(list_U.dimension()==0){
              if(threshold>0.0)
                  #ifdef VERBOSE
          ...
          for(j=U.pointer[k]+1;j<U.pointer[k+1];j++){
              weights_U[U.indices[j]] -= weights_U[perm.get(k)]*U.data[j];
          }
          #ifdef VERBOSE
              time_1=clock();
...
template<class T> void matrix_sparse<T>::weighted_symmetric_move_to_corner(index_list& P) const {
  try {
    ...
    for (i=0;i<n;i++) w.set(i) *= counter.get(i)+pointer[i+1]-pointer[i];
    P.resize(n);
    P.init();
    w.quicksort(P,0,n-1);
  }
...
template<class T> void matrix_sparse<T>::weighted2_symmetric_move_to_corner(index_list& P) const {
  try {
    ...
    for (i=0;i<n;i++) w.set(i) = w.get(i)*(pointer[i+1]-pointer[i]) + colsum.get(i)*colcounter.get(i);
    P.resize(n);
    P.init();
    w.quicksort(P,0,n-1);
  }
...
template<class T> void matrix_sparse<T>::symmetric_move_to_corner_improved(index_list& P) const {
  try {
    ...
    for(i = 0; i < pointer_size-1; i++){
        P.set(i) = w.index_min();
        w.remove_min();
        unused.set(P.get(i)) = false;
        for(j = pointer[P.get(i)]; j<pointer[P.get(i)+1]; j++){
            if(unused.get(indices[j])) w.add(indices[j],fabs(data[j]));
        }
        for(j = A.pointer[P.get(i)]; j<A.pointer[P.get(i)+1]; j++){
...
template<class T> void matrix_sparse<T>::diagonally_dominant_symmetric_move_to_corner_improved(index_list& P) const {
  try {
    ...
    for(i = 0; i < n; i++){
        ...
        acceptable_index = (row_gap.get(current_index)>=0) && (col_gap.get(current_index)>=0);
        j=pointer[current_index];
        while(acceptable_index && j<pointer[current_index+1]){
            if (unused.get(indices[j])==1) acceptable_index = acceptable_index && (fabs(data[j])<= col_gap.get(indices[j]));
            j++;
        }
        j = A.pointer[current_index];
        while(acceptable_index && j<A.pointer[current_index+1]){
            if (unused.get(A.indices[j])==1) acceptable_index = acceptable_index && (fabs(A.data[j])<= row_gap.get(A.indices[j]));
            j++;
        }
        if(acceptable_index){
            P.set(counter) = current_index;
            w.remove_min();
            unused.set(current_index) = 1;
            for(j = pointer[current_index]; j<pointer[current_index+1]; j++){
                if(equal_to_zero(unused.get(indices[j]))) w.add(indices[j],fabs(data[j]));
                col_gap.set(indices[j]) -= fabs(data[j]);
                //if(current_index != indices[j]) col_gap.set(indices[j]) -= fabs(data[j]);
            }
            for(j = A.pointer[current_index]; j<A.pointer[current_index+1]; j++){
                if(equal_to_zero(unused.get(A.indices[j]))) w.add(A.indices[j],fabs(A.data[j]));
                row_gap.set(A.indices[j]) -= fabs(A.data[j]);
                //if(current_index != A.indices[j]) row_gap.set(A.indices[j]) -= fabs(A.data[j]);
            }
            counter++;
        } else {
            unused.set(current_index) = -1;
    ...
    for(i=0;i<counter;i++) w.remove(P.get(i));
    for(i=0;i<n;i++){
        if(unused.set(i) == -1){
            for(j=pointer[i];j<pointer[i+1];j++) if(unused.get(indices[j])!=-1) w.add(i,fabs(data[j]));
            for(j=A.pointer[i];j<A.pointer[i+1];j++) if(unused.get(A.indices[j])!=-1) w.add(i,fabs(A.data[j]));
        }
    }
    for(i=counter;i<n;i++){
        P.set(i) = w.index_min();
        w.remove_min();
        unused.set(P.get(i)) = 0;
        for(j = pointer[P.get(i)]; j<pointer[P.get(i)+1]; j++){
            if(unused.get(indices[j])==-1) w.add(indices[j],fabs(data[j]));
        }
        for(j = A.pointer[P.get(i)]; j<A.pointer[P.get(i)+1]; j++){
...
template<class T> void matrix_sparse<T>::weighted_symmetric_move_to_corner_improved(index_list& P) const {
  try {
    ...
    matrix_sparse<T> A;
    A.change_orientation_of_data(*this);
    for(i=0;i<n;i++) counter.set(i)=pointer[i+1]-pointer[i]+A.pointer[i+1]-A.pointer[i];
    for(i = 0; i < pointer_size-1; i++){
        P.set(i) = w.index_min();
        w.remove_min();
        unused.set(P.get(i)) = false;
        for(j = pointer[P.get(i)]; j<pointer[P.get(i)+1]; j++){
            if(unused.get(indices[j])) w.add(indices[j],counter.get(indices[j])*fabs(data[j]));
        }
        for(j = A.pointer[P.get(i)]; j<A.pointer[P.get(i)+1]; j++){
...
template<class T> void matrix_sparse<T>::weighted2_symmetric_move_to_corner_improved(index_list& P) const {
  try {
    ...
    matrix_sparse<T> A;
    A.change_orientation_of_data(*this);
    for(i=0;i<n;i++) counterrows.set(i)=pointer[i+1]-pointer[i];
    for(i=0;i<n;i++) countercols.set(i)=A.pointer[i+1]-A.pointer[i];
    for(i = 0; i < pointer_size-1; i++){
        P.set(i) = w.index_min();
        w.remove_min();
        unused.set(P.get(i)) = false;
        for(j = pointer[P.get(i)]; j<pointer[P.get(i)+1]; j++){
            if(unused.get(indices[j])) w.add(indices[j],countercols.get(indices[j])*fabs(data[j]));
        }
        for(j = A.pointer[P.get(i)]; j<A.pointer[P.get(i)+1]; j++){
...
template<class T> void matrix_sparse<T>::sp_symmetric_move_to_corner_improved(index_list& P) const {
  try {
    ...
    for(i = 0; i < pointer_size-1; i++){
        ...
        w.remove_min();
        unused.set(P.get(i)) = false;
        posA = A.pointer[P.get(i)];
        for(j = pointer[P.get(i)]; j<pointer[P.get(i)+1]; j++){
            while(A.indices[posA] < indices[j] && posA < A.pointer[P.get(i)+1]) posA++;
            //if ( unused.get(indices[j]) &&  (A.indices[posA] == indices[j] && data[j]*A.data[posA] > 0.0) )   w.add(indices[j],data[j]*A.data[posA]);
...
template<class T> void matrix_sparse<T>::symb_symmetric_move_to_corner_improved(index_list& P) const {
  try {
    ...
    for(i = 0; i < pointer_size-1; i++){
        ...
        unused.set(P.get(i)) = false;
        for(j = pointer[P.get(i)]; j<pointer[P.get(i)+1]; j++){
            if(unused.get(indices[j])) w.add(indices[j],1.0);
        }
        for(j = A.pointer[P.get(i)]; j<A.pointer[P.get(i)+1]; j++){
...
template<class T> void matrix_sparse<T>::non_negative_diagonal(vector_dense<T>& D1) const {
  try {
      ...
      for(Integer k=0; k<pointer_size-1; k++)
          for(Integer j=pointer[k]; j<pointer[k+1]; j++)
              if(indices[j]==k && data[j]<0) D1.set(k)=-1.0;
...
template<class T> void matrix_sparse<T>::permute(const matrix_sparse<T>& A, const index_list& perm)  {
  try {
    ...
    for (i=0; i<dim_along_orientation(); i++){
        pointer[i] = counter;
        for(j=A.pointer[perm.get(i)]; j<A.pointer[perm.get(i)+1]; j++){
            data[counter] = A.data[j];
...
template<class T> void matrix_sparse<T>::permute_against_orientation_with_invperm(const matrix_sparse<T>& A, const index_list& invperm)  {
  try {
    ...
    for (i=0; i<dim_along_orientation(); i++){
        for(j=A.pointer[i]; j<A.pointer[i+1]; j++){
            indices[j] = invperm.get(A.indices[j]);
            data[j]= A.data[j];
        }
...
template<class T> void matrix_sparse<T>::permute_along_with_perm_and_against_orientation_with_invperm(const matrix_sparse<T>& A, const index_list& perm_along, const index_list& invperm_against){
  try {
    ...
    for (i=0; i<dim_along_orientation(); i++){
        pointer[i] = counter;
        for(j=A.pointer[perm_along.get(i)]; j<A.pointer[perm_along.get(i)+1]; j++){
            data[counter] = A.data[j];
            indices[counter] = invperm_against.get(A.indices[j]);
            counter++;
        }
    }
    pointer[pointer_size-1]=counter;
...
template<class T> int matrix_sparse<T>::pardiso_maximal_weight(index_list& P, vector_dense<T>& D1, vector_dense<T>& D2) const {
  ...
  try {
    ...
    for(i=0;i<neqns;i++) P.set(i)  = Perm[i];
    for(i=0;i<neqns;i++) D1.set(i) = std::exp(scale1[i]);
    for(i=0;i<neqns;i++) D2.set(i) = std::exp(scale2[i]);
...
void index_list::quicksort_with_inverse(index_list& invperm, Integer left, Integer right){
     ...
     if(left<right){
         ...
         while(i<=j){
             ...
             while(indices[j]>m) j--;
             if(i<=j){
                 invperm.switch_index(get(i),get(j));
                 switch_index(i,j);
                 i++;
                 j--;
             }
