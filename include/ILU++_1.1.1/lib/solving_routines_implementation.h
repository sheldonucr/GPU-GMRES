          ...
          Real& time, std::string directory, std::string filename, std::string matrix_name, std::string precond_name)
{
    try {
          ...
          if(!outfile){
              std::cerr<<"solve_linear_system: error opening file to write. Returning."<<std::endl;
              return false;
          }
          ...
          outfile.setf(std::ios::right|std::ios::fixed);
          if(exact_solution_known) outfile<<std::setw(6)<<std::setprecision(2)<<abs_error<<" | ";
          else outfile<<"unknown| ";
          outfile.setf(std::ios::right|std::ios::fixed);
          ...
          if(iteration_successful) outfile<<"success|";
          else                     outfile<<"failure|";
          outfile<<std::endl;
          if(iteration_successful)return true;
          else return false;
    }
