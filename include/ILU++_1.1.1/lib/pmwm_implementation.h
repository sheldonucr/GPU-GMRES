...
template<class sparse_matrix_class, class permutation>
	void sapTree<sparse_matrix_class, permutation>::destroy_fields() {
		row_pointer.destroy();
		cand_weights.destroy();
		weights.destroy();
	}
	
template<class sparse_matrix_class, class permuation>
	void sapTree<sparse_matrix_class, permuation>::destroy_resize_fields(Integer size) {
		try {
			destroy_fields();
			row_pointer.erase_resize_data_field(size);
			cand_weights.erase_resize_data_field(size);
			weights.erase_resize_data_field(size);
		}
		catch (iluplusplus_error ippe) {
			std::cerr << "sapTree::destroy_resize_fields: "<<ippe.error_message()<<" Returning."<< std::endl;
			destroy_fields();
			throw;
		}
	}
...
// Copy constructor
template<class sparse_matrix_class, class permutation>
	sapTree<sparse_matrix_class, permutation>::sapTree(const sapTree<sparse_matrix_class, permutation>& tree) {
		
		try {
			root = tree.root;
			lsap = tree.lsap;
			cand_nodes = tree.cand_nodes;
			checked_nodes = tree.checked_nodes;
			reduced_dist = tree.reduced_dist;
			
			// copy fields cand_weights, weights
			Integer dim = checked_nodes.dimension();
			destroy_resize_fields(dim);
			for (Integer k = 0; k < dim; k++) {
				row_pointer[k] = tree.row_pointer[k];
				cand_weights[k] = tree.cand_weights[k];
				weights[k] = tree.weights[k];
			}
		}
		catch (std::bad_alloc) {
			std::cerr << "sapTree::sapTree(sapTree): Error allocating memory." << std::endl;
			destroy_fields();
			throw;
		}
		catch (iluplusplus_error ippe) {
			std::cerr << "sapTree::sapTree(sapTree): " << ippe.error_message() << std::endl;
			destroy_fields();
			throw;
		}
	}
...
// Destructor
template<class sparse_matrix_class, class permutation>
	sapTree<sparse_matrix_class, permutation>::~sapTree() {
		destroy_fields();
	}


//******************************************************************************************************************************
// Class sapTree: Operators                                                                                                    *
//******************************************************************************************************************************

template<class sparse_matrix_class, class permutation>
	sapTree<sparse_matrix_class, permutation>& sapTree<sparse_matrix_class, permutation>::operator =(const sapTree<sparse_matrix_class, permutation>& tree) {
		try {
...
template<class sparse_matrix_class, class permutation>
	void sapTree<sparse_matrix_class, permutation>::resize(Integer dim) {
		...
		try {
			...
			std::cerr << "sapTree::sapTree: Error allocating memory." << std::endl;
			destroy_fields();
			throw;
		}
		catch (iluplusplus_error ippe) {
			std::cerr << "sapTree::sapTree: " << ippe.error_message() << std::endl;
			destroy_fields();
			throw;
		}
	}
...
template<class sparse_matrix_class, class permutation>
	void sapTree<sparse_matrix_class, permutation>::augment(permutation& mate_row, permutation& mate_col, Integer i, Integer j) {
		...
		try {
			...
			std::cerr << "sapTree::augment: " << ippe.error_message() << std::endl;
			destroy_fields();
			throw;
		}
	}
...
template<class sparse_matrix_class, class permutation>
	void sapTree<sparse_matrix_class, permutation>::dual_initialization(const sparse_matrix_class& A,  const array<Real>& comp, array<Real>& u, array<Real>& v) {
		try {
			...
			std::cerr << "sapTree::dual_initialization: " << ippe.error_message() << std::endl;
			destroy_fields();
			throw;
		}
	}
...
template<class sparse_matrix_class, class permutation>
	void sapTree<sparse_matrix_class, permutation>::matching_initialization(const sparse_matrix_class& A, permutation& mate_row, permutation& mate_col, const array<Real>& comp, const array<Real>& u, const array<Real>& v) {
		try {
			...
			std::cerr << "sapTree::matching_initialization: " << ippe.error_message() << std::endl;
			destroy_fields();
			throw;
		}
	}
...
template<class sparse_matrix_class, class permutation>
	void sapTree<sparse_matrix_class, permutation>::dual_update(const sparse_matrix_class& A, const permutation& mate_row, const permutation& mate_col, array<Real>& u, array<Real>& v, Integer isap, Integer jsap) {
		...
		try {
			...
			std::cerr << "sapTree::dual_update: " << ippe.error_message() << std::endl;
			destroy_fields();
			throw;
		}
	}
...
template<class sparse_matrix_class, class permutation>
	void sapTree<sparse_matrix_class, permutation>::find_sap(const sparse_matrix_class& A, const permutation& mate_col,  const array<Real>& comp, const array<Real>& u, const array<Real>& v, Integer& isap, Integer& jsap) {
		...
		try {
			...
			std::cerr << "sapTree::find_sap: " << ippe.error_message() << std::endl;
			destroy_fields();
			throw;
		}
		catch (std::bad_alloc) {
			std::cerr << "sapTree::find_sap: Error allocating memory." << std::endl;
			destroy_fields();
			throw iluplusplus_error(INSUFFICIENT_MEMORY);
		}
		catch (...) {
			std::cerr << "sapTree::find_sap: Unknown Error." << std::endl;
			destroy_fields();
...
template<class sparse_matrix_class>
	void abs_max_row(const sparse_matrix_class& A, array<Real>& max) {
		try {
			...
			std::cerr << "abs_max_row: " << ippe.error_message() << std::endl;
			throw;
		}
	}			
...
template<class sparse_matrix_class>
	void transform_and_copy_data(const sparse_matrix_class& A, array<Real>& comp, array<Real>& max) {
		try {
			...
			std::cerr << "transform_and_copy_data: " << ippe.error_message() << std::endl;
			throw;
		}
	}
...
template<class sparse_matrix_class, class permutation, class vector>	
	bool find_pmwm(const sparse_matrix_class& A, permutation& mate_row, permutation& mate_col, vector& inverse_row_scaling, vector& inverse_col_scaling) {
		...
		try {
			...
			std::cerr << "find_pmwm: " << ippe.error_message() << std::endl;
			u.destroy();
			v.destroy();
			comp.destroy();
			return false;
			...
			std::cerr << "find_pmwm: unknown error"  << std::endl;
			u.destroy();
			v.destroy();
			comp.destroy();
			return false;
...
template<class sparse_matrix_class, class permutation>
	void column_perm(const sparse_matrix_class& A, permutation& col_perm) {
		...
		try {
			...
			std::cerr << "col_perm: " << ippe.error_message() << std::endl;
			throw;
		}
	}
