...
exit(1);
    cout<<"*****************************************************************************"<<endl;
    cout<<"Examples for complex matrices.... Essentially, nothing changes...."<<endl;
    cout<<"*****************************************************************************"<<endl;

    cAcol.read_hb("./hb_matrices/young1c.rb");
    cArow.change_orientation_of_data(cAcol);

    iluplusplus::multilevelILUCDPPreconditioner<Complex,Complex_Matrix,Complex_Vector> cPr;

    param.set_threshold(1.0);

    cPr.make_preprocessed_multilevelILUCDP(cAcol,param);
    ...
    if(cArow.rows()<50){
        ...
        cout<<"Matrices, permutations and scalings used:"<<endl<<endl;
        cout<<endl;
        for(int k=0;k<cPr.levels();k++){
            cout<<"   *********************************** "<<endl;
            cout<<"       Information on Level "<<k<<endl;
            cout<<"   *********************************** "<<endl;
            cout<<"            L:"<<endl<<cPr.extract_left_matrix(k).expand();
            cout<<"            U:"<<endl<<cPr.extract_right_matrix(k).expand();
            cout<<"            D^{-1}:"<<endl<<cPr.extract_middle_matrix(k);
            cout<<"            Row Permutation:"<<endl<<cPr.extract_permutation_rows(k);
            cout<<"            Column Permutation:"<<endl<<cPr.extract_permutation_columns(k);
            cout<<"            Inverse of Row Permutation:"<<endl<<cPr.extract_inverse_permutation_rows(k);
            cout<<"            Inverse of Column Permutation:"<<endl<<cPr.extract_inverse_permutation_columns(k);
            cout<<"            Scaling of Rows:"<<endl<<cPr.extract_left_scaling(k);
            cout<<"            Scaling of Columns:"<<endl<<cPr.extract_right_scaling(k)<<endl;
        }
    } else
        cout<<"Matrix is too large to print preconditioner."<<endl;

    cout<<"fill-in: "<<((Real) cPr.total_nnz())/(Real)cAcol.non_zeroes()<<endl;

    cout<<endl<<flush;

    Complex_Vector cb,cx_exact,cxbicgstab,cxcgs,cxgmres;

    cx_exact.resize(cAcol.rows(),1.0);

    cAcol.matrix_vector_multiplication(iluplusplus::ID,cx_exact,cb);
    ...
    cout<<"BiCGstab"<<endl;
    cout<<"*****************************************************************************"<<endl;
    iluplusplus::bicgstab<Complex,Complex_Matrix,Complex_Vector>(cPr,iluplusplus::SPLIT,cAcol,cb,cxbicgstab,min_iter,max_iter,rel_tol,abs_tol,true);
    cout<<"Iterations "<<max_iter<<endl;
    cout<<"error: "<<(cxbicgstab-cx_exact).norm_max()<<endl<<flush;
    cout<<"relative decrease in norm of residual: "<<exp(-rel_tol*log(10.0))<<endl;
    cout<<"absolute residual: "<<exp(-abs_tol*log(10.0))<<endl;
    ...
    cout<<"CGS"<<endl;
    cout<<"*****************************************************************************"<<endl;
    iluplusplus::cgs<Complex,Complex_Matrix,Complex_Vector>(cPr,iluplusplus::SPLIT,cAcol,cb,cxcgs,min_iter,max_iter,rel_tol,abs_tol,true);
    cout<<"Iterations "<<max_iter<<endl;
    cout<<"error: "<<(cxcgs-cx_exact).norm_max()<<endl<<flush;
    cout<<"relative decrease in norm of residual: "<<exp(-rel_tol*log(10.0))<<endl;
    cout<<"absolute residual: "<<exp(-abs_tol*log(10.0))<<endl;
    ...
    iluplusplus::gmres<Complex,Complex_Matrix,Complex_Vector>(cPr,iluplusplus::SPLIT,cAcol,cb,cxgmres,restart,min_iter,max_iter,rel_tol,abs_tol,true);
    cout<<"Iterations "<<max_iter<<endl;
    cout<<"error: "<<(cxgmres-cx_exact).norm_max()<<endl<<flush;
    cout<<"relative decrease in norm of residual: "<<exp(-rel_tol*log(10.0))<<endl;
    cout<<"absolute residual: "<<exp(-abs_tol*log(10.0))<<endl;
    cout<<endl;
